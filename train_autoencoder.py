...
def read_flag():
    ...
    parser.add_argument('--encode-conv-filters', type=tuple, default=ENCODE_CONV_FILTERS,
                        help='#filters at each 1d convolution')
    parser.add_argument('--encode-fc-filters', type=tuple, default=ENCODE_FC_FILTERS,
                        help='#neurons in each fully connected layers in encoder')
...
def main(flags):
    ...
    # make network
    ntwk = network_maker.AutoEncoderNetwork(features, labels, utils.my_model_fn, flags.batch_size,
                                            fc_filters=flags.fc_filters, tconv_dims=flags.tconv_dims,
                                            tconv_filters=flags.tconv_filters,
                                            encode_conv_filters=flags.encode_conv_filters,
                                            encode_fc_filters=flags.encode_fc_filters,
                                            learn_rate=flags.learn_rate,
                                            decay_step=flags.decay_step, decay_rate=flags.decay_rate)
    # define hooks for monitoring training
    train_e_hook = network_helper.TrainValueHook(flags.verb_step, ntwk.loss[0], ckpt_dir=ntwk.ckpt_dir,
                                                 write_summary=True, value_name='e_loss')
    train_g_real_hook = network_helper.TrainValueHook(flags.verb_step, ntwk.loss[1], ckpt_dir=ntwk.ckpt_dir,
                                                      write_summary=True, value_name='g_loss_real')
    ...
    # train the network
    ntwk.train(train_init_op, flags.train_step,
               [train_e_hook, train_g_real_hook, train_g_fake_hook, valid_hook, lr_hook],
               write_summary=True)
