...
def main(flags):
    ...
    # make network
    ntwk = network_maker.BiganNetwork(features, labels, utils.my_model_fn, flags.batch_size,
    ...
    train_g_hook = network_helper.TrainValueHook(flags.verb_step, ntwk.loss[0], ckpt_dir=ntwk.ckpt_dir,
                                                 write_summary=True, value_name='g_loss')
    train_d_hook = network_helper.TrainValueHook(flags.verb_step, ntwk.loss[1], ckpt_dir=ntwk.ckpt_dir,
                                                 write_summary=True, value_name='d_loss')
    train_e_hook = network_helper.TrainValueHook(flags.verb_step, ntwk.loss[2], ckpt_dir=ntwk.ckpt_dir,
    ...
    valid_hook = network_helper.ValidationHook(flags.eval_step, valid_init_op, ntwk.labels, ntwk.logits[2], ntwk.loss[0],
                                               ckpt_dir=ntwk.ckpt_dir, write_summary=True)
    # train the network
    ntwk.train(train_init_op, flags.train_step, [train_g_hook, train_d_hook, train_e_hook, valid_hook, lr_hook],
               write_summary=True)
